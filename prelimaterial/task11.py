...
class Player:
    ...
    def DeductChance(self):
        self._Chances -= 1

    def GetChances(self):
        return self._Chances

    def ResetChances(self):
        self._Chances = 3
...
def PlayGame(Player1, Player2, Grid):
    ...
    while not (GameOver and Player1Turn):
        ...
        for C in Commands:
            ...
            if not ValidCommand and (
                (Player1.GetChances() == 0 and Player1Turn)
                or (Player2.GetChances() == 0 and not Player1Turn)
            ):
                print("Invalid command")
            else:
                if not ValidCommand:
                    print("Command number", Commands.index(C), "is an invalid command")
                    if Player1Turn:
                        Player1.DeductChance()
                        print("You have", Player1.GetChances(), "remaining")
                    else:
                        Player2.DeductChance()
                        print("You have", Player2.GetChances(), "remaining")
                    Command = input(f"enter new command {Commands.index(C)}:")
                    Items = Command.split(" ")
                FuelChange = 0
                LumberChange = 0
                SupplyChange = 0
