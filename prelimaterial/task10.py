...
class HexGrid:
    ...
    def ExecuteCommand(self, Items, FuelAvailable, LumberAvailable, PiecesInSupply):
        ...
        if Items[0] == "move":
            ...
            LumberCost = self.__ExecuteSalvageCommand(Items, PiecesInSupply)
            if LumberCost < 0:
                return "could not salvage", FuelChange, LumberChange, SupplyChange
            LumberChange = LumberCost
    ...
    def __ExecuteSalvageCommand(self, Items, PiecesInSupply):
        TileToUse = int(Items[1])
        if not self.__CheckPieceAndTileAreValid(TileToUse):
            return -1
        ThePiece = self._Tiles[TileToUse].GetPieceInTile()
        if ThePiece.GetPieceType().upper() not in ["S", "P", "L"]:
            return -1
        ThePiece.DestroyPiece()
        PiecesInSupply += 1
        return 5
