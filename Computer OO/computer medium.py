    ...
    def recv_control(self, c):
        if c == Bus.READ:
            ...
            if self.__send_data:
                ...
                raise Exception("data bus not connected to ram")
        elif c == Bus.WRITE:
        ...
        # TODO 10) Raise exception if command is not READ or WRITTE
            ...
            raise Exception("bus isn't read or write")
...
class Cpu(Named):
    ...
    def __init__(self, name):
        ...
        self.__pc = Register("PC", 8)
        self.__cir = Register("CIR", 17)
        self.__mar = Register("MAR", 8)
        self.__mbr = Register("MBR")
        self.__registers = [Register(f"R{n}") for n in range(16)]
        self.__status = Register("Status", 4)
        self.__ticks = 0
...
{regs}"""
    ...
    def __fetch(self):
        ...
        self.__cir.value = self.__mbr.value
    ...
    def __execute(self):
        ...
        elif self.__opcode == Cpu.MOV:
            rd = extractdrd()
            if self.__addrmode == Cpu.IMMEDIATE:
                self.mar.value = self.__operand & 0xFF
                self.mbr.value = self.__registers[rd].value
                self.__send_control(Bus.WRITE)
            else:
                raise Exception("Cannot use direct addressing for store")
            self.__state = Cpu.FETCH
    ...
    def clock(self):
        ...
        # TODO 6) Call the fetch/decode/execute pipeline stage depending on state (use match/case if you have Python 10)
...
class System:
    def __init__(self):
        ...
        # TODO 8) Once Bit Widths are implemented, set the address bus to 8-bits, the data bus to 32-bits and the control bus to 1-bit
        A = Bus(Bus.ADDRESS, 8)
        D = Bus(Bus.DATA, 32)
        C = Bus(Bus.CONTROL, 1)
...
R0, R1, R2, X, Y, Z = 0, 1, 2, 10, 11, 7
S13, S12, S8, S4 = 13, 12, 8, 4
...
program = [
    ...
    | (Cpu.IMMEDIATE << S12)
    | (R0 << S8)
    | X,  # 0 : LDR r0, x (address 5)
    (Cpu.LDR << S13)
    | (Cpu.IMMEDIATE << S12)
    | (R1 << S8)
    | Y,  # 1 : LDR r1, y (address 6)
    (Cpu.ADD << S13)
    | (Cpu.DIRECT << S12)
    | (R2 << S8)
    | (R1 << S4)
